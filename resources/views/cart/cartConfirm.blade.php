            ...
            <tbody>
                ...
                <tr>
                    <td class="table-image">
                        @if ($prod_layout == 'NTAG')
                        <a href="{{ url(substr_replace($item->options->proofPath, 'pdf', -3)) }}" target="_blank"><img
                                src="/{{ $item->options->proofPath }}" style="max-width:300px;" alt="proof"
                                class="img-responsive cart-image move-right"></a>
                        @else
                        <a href="{{ url(substr_replace($item->options->proofPath, 'pdf', -3)) }}" target="_blank"><img
                                src="/{{ $item->options->proofPath }}" style="max-width:300px;" alt="proof"
                                class="img-responsive cart-image move-right dropshadow"></a>
                        @endif
                    </td>
                    ...
                    <td>
                        ...
                        @if ($prod_layout == 'NTAG')
                        {!! Form::select('qty', array('Select Quantity', '1' => '1 Name Badge', '2' => '2 Name Badges',
                        '3' => '3 Name Badges'), ['class' => 'quantity move-down'],
                        ['style' => 'font-size:12px']) !!}
                        @endif

                        @if ($prod_layout == 'SBC' || $prod_layout == 'ABC' || $prod_layout == 'PBC' || $prod_layout ==
                        'ADSBC' || $prod_layout == 'PDSBC')
                        {!! Form::select('qty', array('Select Quantity', '250' => '250', '500' => '500'), ['class' =>
                        'quantity move-down'], ['style' => 'font-size:12px']) !!}
                        @endif

                        @if ($prod_layout == 'SFYI' || $prod_layout == 'AFYI' || $prod_layout == 'PFYI')
                        {!! Form::select('qty', array('Select Quantity', '4' => '4 Pads', '8' => '8 Pads'), ['class' =>
                        'quantity move-down'], ['style' => 'font-size:12px']) !!}
                        @endif

                        @if ($prod_layout == 'SBCFYI' || $prod_layout == 'ABCFYI' || $prod_layout == 'PBCFYI')
                        {!! Form::select('qty', array('Select Quantity', '24' => '250 BCs + 4 Pads', '28' => '250 BCs +
                        8 Pads', '54' => '500 BCs + 4 Pads', '58' => '500 BCs + 8 Pads',), ['class' => 'quantity
                        move-down'], ['style' => 'font-size:12px']) !!}
                        @endif
                        ...
                        default: $bcfyi_qty = '1 Name Badge';
                        }
                        }
                        if ($prod_layout == 'SBCFYI' || $prod_layout == 'ABCFYI' || $prod_layout == 'PBCFYI') {
                        switch ($item->qty) {
        ...
        <div class="col-sm-12 col-md-12 col-lg-12">
            ...
            <div class="thumbnail">
                ...
                <div class="caption">
                    ...
                    <h4 class='move-up'>This is a <strong>RUSH order, with expedited production, and delivery
                            ASAP</strong><br><br>It will be shipped to:</h4>
                    @else
                    <h4 class='move-up'>This is a <strong>RUSH order, with an expected delivery date by
                            {{ Session::get('dt_rush') }}</strong><br><br>It will be shipped to:</h4>
                    @endif
                    @endif

                    <strong>{{ Auth::user()->loc_name }}</strong>&nbsp;&nbsp;<small><a
                            href="{{route('showProfile')}}">(change shipping info)</small></a><br>
                    ...
                    <p><small>
                            An Email confirmation will be sent to the admin: {{ Auth::user()->contact_a }}
                    ...
                    <form action="{{ route('cartorder') }}" method="POST">
                                ...
                                <div class="col-lg-10 col-md-10 col-sm-10">
                                    <div class="thumbnail">
                                        <p class="move-down">
                                            ...
                                            <input type="checkbox" name="confirm">&nbsp;&nbsp;I have reviewed the
                                            Proof(s) of my cart item(s) and confirm that it is correct. Other than Name Badges, or if I have
                                            specifically instructed to the contrary, I understand that production will
                                            commence upon submission, and will be shipped without delay.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        {!! csrf_field() !!}
                        <input type="submit" class="btn btn-success col-md-5 offset-md-4 pull-left"
                            value="Yes, Place my Order">
                    </form>
                    {{-- </div> --}}

                    {{-- <a href="{{view('cart/cartOrder')}}" class="col-md-5 offset-md-4 btn btn-success"
                    role="button"> Yes, Place my Order </a> --}}
                    &nbsp;
                    <a href="/cart" class="col-md-5 offset-md-4 btn btn-danger pull-right" role="button"> No, Return to
                        my Cart </a>
                    </p>
                </div>
            </div>
